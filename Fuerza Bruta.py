...
# Función para encontrar el camino más corto entre dos puntos utilizando fuerza bruta
def find_shortest_path(start, end):
    points_list = list(points.keys())
    permutations = get_permutations(points_list)
    shortest_path = None
    min_distance = float('inf')
    
    for permutation in permutations:
        if permutation[0] == start and permutation[-1] == end:
...
# Función para obtener todas las permutaciones posibles de una lista
